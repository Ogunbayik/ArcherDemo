    ...
    public override void ExecuteAttack(EnemyBase enemy)
    {
        ...
        for (int i = 0; i < prefabCount; i++)
        {
            ...
            if (isEvenPrefab == 0)
            {
                ...
                {
                    if(isEvenIndex == 0)
                        spawnLocalPosition = enemy.GetAttackTransform().localPosition + new Vector3(distanceToPrefab / prefabCount, 0f, 0f);
                    else
                        spawnLocalPosition = enemy.GetAttackTransform().localPosition + new Vector3(-distanceToPrefab / prefabCount, 0f, 0f);
                }
                else
                {
                    if (isEvenIndex == 0)
                        spawnLocalPosition = enemy.GetAttackTransform().localPosition + new Vector3(distanceToPrefab * distanceMultiplier, 0f, 0f);
                    else
                        spawnLocalPosition = enemy.GetAttackTransform().localPosition + new Vector3(-distanceToPrefab * distanceMultiplier, 0f, 0f);
                }
            }
            ...
            prefab.transform.position = enemy.GetAttackTransform().TransformPoint(spawnLocalPosition);
